...
│   └── ...
├── Share/                 # 共享工具库
│   ├── RingBuffer.hpp     # 无锁环形缓冲区
│   ├── charconv.hpp       # 高性能字符串转换
│   └── ...
├── WtCore/                # 核心引擎 (CTA/HFT/SEL)
│   ├── WtEngine.h         # 引擎基类
│   ├── WtHftEngine.h      # 高频引擎
│   └── ...
├── WtUftCore/             # UFT极速引擎核心
│   ├── WtUftEngine.h      # UFT引擎
│   ├── UftStraContext.h   # 策略上下文
│   └── TraderAdapter.h    # 交易适配器
├── WtFutuCore/            # 期货做市引擎 (GLFT+Alpha)
│   ├── UftFutuMmStrategy.h    # 做市策略主类
│   ├── MicroAlphaEngine.h     # Alpha预测引擎
│   ├── ToxicFlowDetector.h    # 毒性检测
│   ├── TickTransactionInferer.h   # Tick推断器
│   ├── SelfTradeCalibrator.h  # 自身成交校准
│   ├── SyntheticSignalFusion.h # 信号融合
│   └── ...
├── WtOptionCore/          # 期权做市引擎
├── API/                   # 交易接口封装
│   ├── CTP6.3.15/         # 上期CTP接口
...
| 引擎 | 适用场景 | 延迟 | 驱动模式 |
|------|---------|------|---------|
| **CTA引擎** | 中低频择时、套利 | ~70μs (Python) / ~4.5μs (C++) | 事件+时间驱动 |
| **SEL引擎** | 多因子选股、截面多空 | 秒级 | 时间驱动 |
| **HFT引擎** | 高频策略 | 1-2μs | 事件驱动 |
| **UFT引擎** | 超高频/做市 | <200ns | 事件驱动 |

##### 2. WtFutuCore 期货做市引擎

`WtFutuCore` 是基于 UFT 框架的高频做市引擎，实现 GLFT+Alpha 做市框架：
...
│                                                             │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │ FutuPortfolio│  │ FutuQuoter │  │ SpreadOptimizer     │  │
│  │ (组合管理)   │  │ (报价执行)  │  │ (GLFT价差优化)       │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
│                                                             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │              MicroAlphaEngine (Alpha预测)             │  │
│  │  α = w_ofi × α_ofi + w_trade × α_trade + w_ll × α_ll │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │          SyntheticSignalFusion (综合信号融合)         │  │
│  │  用于无逐笔成交数据的市场 (如国内期货)                  │  │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────────────┐   │  │
│  │  │TickInferer│ │BookAnalyzer│ │SelfTradeCalibrator│   │  │
│  │  │(Tick推断) │ │(订单簿分析)│ │(自身成交校准)      │   │  │
│  │  └───────────┘ └───────────┘ └───────────────────┘   │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │           ToxicFlowDetector (毒性检测)              │   │
│  │  检测逆向选择风险，触发时展宽价差或暂停报价            │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| 组件 | 功能 | 文件 |
|------|------|------|
| `UftFutuMmStrategy` | 做市策略主类，协调各模块 | `UftFutuMmStrategy.h` |
| `FutuPortfolio` | 组合风险管理、Delta计算、对冲 | `FutuPortfolio.h` |
| `FutuQuoter` | 多档位报价执行引擎 | `FutuQuoter.h` |
| `SpreadOptimizer` | GLFT价差优化器 | `SpreadOptimizer.h` |
| `MicroAlphaEngine` | Alpha预测引擎 (OFI/Trade/LeadLag) | `MicroAlphaEngine.h` |
| `ToxicFlowDetector` | 毒性订单流检测 | `ToxicFlowDetector.h` |
| `SelfTradeCalibrator` | 自身成交校准 (Ground Truth) | `SelfTradeCalibrator.h` |
| `TickTransactionInferer` | Tick级交易推断 | `TickTransactionInferer.h` |
| `SyntheticSignalFusion` | 多源信号融合 | `SyntheticSignalFusion.h` |
| `MarketStateDetector` | 市场状态检测 | `MarketStateDetector.h` |
| `AutoCancelPolicy` | 自动撤单策略 | `AutoCancelPolicy.h` |
| `SpreadArbitrageManager` | 跨期价差套利 | `SpreadArbitrageManager.h` |

---

#### 配置参数详解

##### 综合信号配置 (syntheticTransaction)
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `enabled` | bool | true | 是否启用综合推断 |
| `tickWeight` | double | 0.4 | Tick推断信号权重 (0-1) |
| `bookWeight` | double | 0.4 | 订单簿信号权重 (0-1) |
| `selfTradeWeight` | double | 0.2 | 自身成交校准权重 (0-1) |
| `minSamples` | int | 5 | 自身成交最小样本数 |

**权重说明：**
- 三者权重之和应为1.0，系统自动归一化
- `selfTradeWeight` 作为 Ground Truth，权重越高越依赖历史成交
- 高波动市场建议降低 `tickWeight`，提高 `bookWeight`
- 自适应模式下权重会根据市场状态动态调整
...
```yaml
alphaEngine:
  sensitivity: 0.5        # η: Alpha对价格影响系数
  ofiWeight: 0.4          # OFI因子权重
  tradeWeight: 0.3        # Trade Imbalance权重
  leadlagWeight: 0.3      # 跨品种LeadLag权重
...
| 参数 | 说明 |
|------|------|
| `sensitivity` | Alpha信号对报价价格的影响系数 η，FairValue = Mid + η × α |
| `ofiWeight` | 订单流不平衡因子权重，反映买卖盘压力 |
| `tradeWeight` | 交易流失衡因子权重，反映主动买卖力量 |
| `leadlagWeight` | 跨品种领滞后因子权重 |
| `emaFactor` | Alpha信号的EMA平滑因子，减少噪声 |
| `strongThreshold` | 强信号阈值，超过后触发单向报价 |

##### 毒性检测配置

```yaml
toxicityDetector:
  vpinThreshold: 0.7      # VPIN毒性阈值
  adverseThreshold: 0.6   # 逆向选择阈值
  alphaWeight: 0.3        # Alpha信号权重
  bookWeight: 0.3         # 订单簿权重
  selfTradeWeight: 0.4    # 自身成交权重 (最高)
```

| 参数 | 说明 |
|------|------|
| `vpinThreshold` | VPIN毒性分数阈值，超过触发熔断 |
| `adverseThreshold` | 逆向选择比例阈值 |
| `alphaWeight` | 预测性毒性(Alpha)的权重 |
| `bookWeight` | 订单簿毒性权重 |
| `selfTradeWeight` | 自身成交毒性权重 (Ground Truth，建议最高) |
...
spreadOptimizer:
  ...
  phi: 0.01               # 库存惩罚系数
  volWindow: 100          # 波动率计算窗口
  minSpreadMult: 0.5      # 最小价差倍数
```

| 参数 | 说明 |
|------|------|
| `volSensitivity` | 波动率对价差的影响敏感度 |
| `depthSensitivity` | 深度对价差的影响敏感度 |
| `phi` | GLFT模型库存惩罚系数 φ |
| `volWindow` | 波动率计算的滚动窗口 |
| `minSpreadMult` | 价差最小倍数 |
...
spreadArbitrage:
  enabled: true           # 启用套利模块
  enhanceMarketMaking: true # 价差信号增强做市
  maxPosition: 20.0       # 单对最大仓位
  entryZScore: 2.0        # 入场Z-Score阈值
  exitZScore: 0.5         # 离场Z-Score阈值
```
