...
struct MmEnhancerConfig
{
    ...
    MmEnhancerConfig()
        : pause_z_threshold(3.0), widen_z_threshold(1.5), skew_z_threshold(1.0), max_skew_adjustment(0.3),
          max_spread_multiplier(2.0), position_skew_factor(0.02), max_position_skew(0.2), min_signal_confidence(0.3),
          signal_weight(0.6), adjustment_decay(0.95), decay_interval_ms(1000)
    {}
};
...
struct QuotingState
{
    ...
    QuotingState()
        : bid_skew(0), ask_skew(0), spread_multiplier(1.0), suppress_bid(false), suppress_ask(false),
          last_adjustment(0), last_adjustment_time(0)
    {}
};
...
public:
    ...
    QuotingAdjustment calculateAdjustment(const SpreadState& state, const SpreadSignal& signal, uint64_t current_time);

    /// Apply signal to current quoting state
    QuotingState applySignal(const SpreadState& state, const SpreadSignal& signal, uint64_t current_time);
