...
void FutuRuntimeOps::processTradeFill(UftFutuMmStrategy& s, wtp::IUftStraCtx* ctx,
...
//   (回测实证: 引擎真账 +34k, 内部账 -3.7M → 日亏 IRREVERSIBLE halt 误杀)
{
    ...
    if (cs_chk && std::abs(cs_chk->position - local_net) > 0.01)
    {
        WTSLogger::warn("UftFutuMmStrategy[{}] position book divergence: {} book={:.0f} engine={:.0f}, resyncing",
            s.id(), stdCode, cs_chk->position, local_net);
...
if (_order_tracker->getOrderInfoCopy(localid, oi_buf)
    && oi_buf.place_time > 0 && _exchange_time_ms >= oi_buf.place_time)
...
_toxicity_detector->onSelfTradeCalibration(calibration);
    ...
    WTSLogger::debug("UftFutuMmStrategy[{}] TRADE: {} {} {}@{} | Delta: {} {}",
        s.id(), stdCode, actionStr, vol, price, _portfolio->getTotalDelta(), effectStr);
    static uint64_t trade_log_cnt = 0;
    if ((++trade_log_cnt % 50) == 1) {
        WTSLogger::info("UftFutuMmStrategy[{}] TRADE[sample #{}]: {} {} {}@{} | Delta: {}",
            s.id(), trade_log_cnt, stdCode, actionStr, vol, price, _portfolio->getTotalDelta());
    }
...
WTSLogger::info("UftFutuMmStrategy[{}] Trading resumed after trade (unified recovery gates passed)", s.id());
}
else if (recovered)
{
...
WTSLogger::debug("UftFutuMmStrategy[{}] Recovery gates not passed after trade, keeping halted state", s.id());
}
}
}
...
s.id(), ci.code, currentPos, local_net);
}
...
if (!has_valid_price)
{
WTSLogger::info("UftFutuMmStrategy[{}] No price yet, resuming quoting (risk will activate on first tick)", s.id());
}
...
const ContractState* breached = _portfolio->getPositionBreachedSnapshot(breached_buf) ? &breached_buf : nullptr;
if (breached)
{
...
double qty = _liquidator.reduceContract(ctx, breached_code, std::abs(reduction), 1,
    "AUTO REDUCE position breach (channel ready)");
if (qty <= 0)
{
WTSLogger::warn("UftFutuMmStrategy[{}] AUTO REDUCE skipped: {} reduction={} (price invalid or no position)",
s.id(), breached_code, reduction);
}
}
}
break;  // 一次只处理一个超限
...
{
WTSLogger::warn("UftFutuMmStrategy[{}] channel ready but risk still exists, keeping halted state", s.id());
}
else
{
WTSLogger::info("UftFutuMmStrategy[{}] Auto position reduction triggered, will retry after trade", s.id());
}
...
// R3 v2: BilateralStats Per-Quoter,逐合约 onSessionEnd + formatString 输出
{
    ...
    _closeout_orch.finalizeAtSessionEnd(_exchange_time_ms > 0 ? _exchange_time_ms.load(std::memory_order_acquire) : TimeUtils::getLocalTimeNow());
...
void FutuRuntimeOps::onEntrust(UftFutuMmStrategy& s, wtp::IUftStraCtx* ctx, uint32_t localid, bool bSuccess, const char* message)
{
auto& _trading_state = s._trading_state;
auto& _order_error_count = s._order_error_count;
...
auto& _last_mid = s._last_mid;
    ...
    WTSLogger::error("UftFutuMmStrategy[{}] Order FAILED (count={}/{}): localid={}, error={}",
        s.id(), _order_error_count, _config.order_control.order_error_threshold, localid, errMsg);
    ...
    if (_order_error_count >= _config.order_control.order_error_threshold)
    {
        ...
        _quoting_paused_since = _exchange_time_ms > 0 ? _exchange_time_ms.load(std::memory_order_acquire) : TimeUtils::getLocalTimeNow();

        WTSLogger::error("UftFutuMmStrategy[{}] Trading HALTED due to consecutive order errors (count={}/threshold={})",
            s.id(), _order_error_count, _config.order_control.order_error_threshold);
...
void FutuRuntimeOps::onOrderEvent(UftFutuMmStrategy& strat, wtp::IUftStraCtx* ctx, uint32_t localid, const char* stdCode,
...
uint64_t now_ms = _exchange_time_ms > 0 ? _exchange_time_ms.load(std::memory_order_acquire) : TimeUtils::getLocalTimeNow();
