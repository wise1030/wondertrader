...
struct SignalResult
{
    ...
    SignalResult(SignalType t) : type(t), confidence(0.0), timestamp(0), valid(false) {}
...
struct OFISignalResult : public SignalResult
{
    ...
    OFISignalResult() : SignalResult(SignalType::OFI), ofi(0), bid_pressure(0.5), ask_pressure(0.5), cumulative_ofi(0)
    {}
};
...
struct VolatilitySignalResult : public SignalResult
{
    ...
    VolatilitySignalResult()
        : SignalResult(SignalType::VOLATILITY), volatility(0), realized_vol(0), composite_vol(0),
          vol_tier(VolTier::NORMAL), vol_percentile(50), vpin(0)
    {}
};
...
struct TradeFlowSignalResult : public SignalResult
{
    ...
    TradeFlowSignalResult()
        : SignalResult(SignalType::TRADE_FLOW), net_flow(0), net_flow_normalized(0), buy_volume(0), sell_volume(0),
...
struct BookImbalanceSignalResult : public SignalResult
{
    ...
    BookImbalanceSignalResult()
        : SignalResult(SignalType::BOOK_IMBALANCE), simple_imbalance(0), depth_imbalance(0), pressure_intensity(0),
          bid_dominant(false), ask_dominant(false), bid_depth(0), ask_depth(0)
    {}
};
...
struct AlphaSignalResult : public SignalResult
{
    ...
    AlphaSignalResult()
        : SignalResult(SignalType::ALPHA), alpha(0), ofi_component(0), trade_component(0), book_imbalance_component(0),
          momentum_component(0), lead_lag_component(0), is_strong_signal(false)
    {}
};
...
struct MarketStateSignalResult : public SignalResult
{
    ...
    MarketStateSignalResult()
        : SignalResult(SignalType::MARKET_STATE), state(MarketState::NORMAL), should_widen(false), should_hedge(false),
          should_pause(false), vol_estimate(0), spread_estimate(0)
    {}
};
...
struct ToxicitySignalResult : public SignalResult
{
    ...
    ToxicitySignalResult()
        : SignalResult(SignalType::TOXICITY), toxicity_score(0), toxic_detected(false), vpin(0), toxic_side(0)
    {}
};
...
struct SignalContext
{
    ...
    bool shouldPause() const { return market_state.should_pause || toxicity.toxic_detected; }

    /// Check if spread should be widened (volatility or state)
    bool shouldWiden() const { return market_state.should_widen || volatility.vol_tier >= VolTier::ELEVATED; }

    /// Check if immediate hedging is needed
    bool shouldHedge() const { return market_state.should_hedge; }

    /// Check for strong alpha signal
    bool isStrongSignal() const { return alpha.is_strong_signal; }
