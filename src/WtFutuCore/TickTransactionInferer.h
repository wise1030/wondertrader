...
struct InferredTransaction
{
    ...
    InferredTransaction()
        : price(0), volume(0), is_buy_initiated(false), is_sell_initiated(false), confidence(0), timestamp(0),
...
struct AccumulatedFlowStats
{
    ...
    AccumulatedFlowStats()
        : buy_volume(0), sell_volume(0), net_flow(0), imbalance_ratio(0), window_start(0), tick_count(0)
    {}
...
struct InferredTradeImbalance
{
    ...
    InferredTradeImbalance() : net_flow(0), imbalance_ratio(0), large_trade_ratio(0), confidence(0), timestamp(0) {}
};
...
struct TickInfererConfig
{
    ...
    TickInfererConfig()
        : imbalance_window_ms(5000), min_confidence(0.3), use_volume_weighting(true), large_trade_threshold(50.0),
          tick_size(1.0)
    {}
};
...
public:
    ...
    InferredTransaction inferFromTick(double bid_px,
                                      double ask_px,
                                      double bid_vol,
                                      double ask_vol,
                                      double last_price,
...
private:
    ...
    struct TickState
    {
        ...
        TickState() : bid_px(0), ask_px(0), bid_vol(0), ask_vol(0), last_price(0), timestamp(0), initialized(false) {}
    };
    ...
    InferenceMethod detectMethod(double bid_px,
                                 double ask_px,
                                 double prev_bid_px,
                                 double prev_ask_px,
                                 double last_price,
                                 double prev_last_price);

    /// Calculate confidence based on method and data quality
    double
    calculateConfidence(InferenceMethod method, double bid_vol_change, double ask_vol_change, double price_change);

    /// Calculate inferred volume from changes
    double
    calculateInferredVolume(double bid_vol, double prev_bid_vol, double ask_vol, double prev_ask_vol, double last_vol);
