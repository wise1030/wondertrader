...
auto require = [&](const void* p, const char* name) {
    if (!p) { WTSLogger::error("[ASSEMBLY] REQUIRED dependency missing: {}", name); dep_errors++; }
};
...
if (_config.modules.use_market_making) {
    if (_quoters.empty()) { WTSLogger::error("[ASSEMBLY] REQUIRED: quoters empty (MM enabled)"); dep_errors++; }
...
if (dep_errors > 0)
    WTSLogger::error("[ASSEMBLY] {} required dependencies MISSING - strategy will misbehave, review assembly order!", dep_errors);
else
...
s.id(), ci.code, ci.multiplier, ci.tick_size, ci.close_time, ci.night_close_time);
}
else
{
