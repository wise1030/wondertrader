...
RecursiveSpinGuard _g(_lock);
    ...
    if (_deps.executor && _deps.executor->isActive())
    {
        ...
        const ContractState* anchorState = _deps.portfolio->getContractSnapshot(*_deps.anchor_code, anchor_buf) ? &anchor_buf : nullptr;
        MarketSnapshot snap;
...
if (_deps.risk_monitor
    && _deps.risk_monitor->getCloseoutSubInfo().is_night_closeout
...
RecursiveSpinGuard _g(_lock);
    ...
    if (isCanceled)
    {
        ...
        if (_deps.order_router)
        {
            ...
            {
                if (o.localid == localid) { total_qty = o.qty; break; }
            }
        }
        ...
        {
            WTSLogger::warn("[CLOSEOUT] Order rejected (zero fill) during flattening: code={} localid={}, marking FAILED for retry",
                stdCode, localid);
            _deps.risk_monitor->markCloseoutFailed(now_ms);
        }
        else
        {
            WTSLogger::info("[CLOSEOUT] Order canceled with partial fill or self-canceled: code={} localid={} left={}/{}, continue executor rounds",
                stdCode, localid, leftQty, total_qty);
        }
    }
    else if (leftQty == 0)
    {
