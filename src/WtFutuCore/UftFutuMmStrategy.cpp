...
void UftFutuMmStrategy::handleQuotingAutoResume()
{
    ...
    uint64_t paused_ms = (_exchange_time_ms > 0 ? _exchange_time_ms.load(std::memory_order_acquire) : TimeUtils::getLocalTimeNow()) - _quoting_paused_since;
...
void UftFutuMmStrategy::handleCoordinatorTick(IUftStraCtx* ctx, const char* stdCode, WTSTickData* tick, uint64_t now_ms)
{
    ...
    if (result.quote_placed && _perf_analyzer)
    {
        ...
        if (mid_it != _last_mid.end()) qmid = mid_it->second->v.load(std::memory_order_acquire);
