...
TickTransactionInferer::TickTransactionInferer() : _large_volume(0), _total_volume(0) {}
...
InferredTransaction TickTransactionInferer::inferFromTick(wtp::WTSTickData* tick)
{
    ...
    return inferFromTick(tickStruct.bid_prices[0],
...
InferredTransaction TickTransactionInferer::inferFromTick(double bid_px,
                                                          double ask_px,
                                                          double bid_vol,
                                                          double ask_vol,
                                                          double last_price,
                                                          ...
                                                          uint64_t timestamp)
{
    ...
    result.method =
        detectMethod(bid_px, ask_px, _prev_state.bid_px, _prev_state.ask_px, last_price, _prev_state.last_price);

    // Calculate inferred volume
    result.volume = calculateInferredVolume(bid_vol,
                                            _prev_state.bid_vol,
                                            ask_vol,
                                            _prev_state.ask_vol,
    ...
    result.confidence = calculateConfidence(result.method, bid_vol_change, ask_vol_change, price_change);
...
InferenceMethod TickTransactionInferer::detectMethod(
    double bid_px, double ask_px, double prev_bid_px, double prev_ask_px, double last_price, double prev_last_price)
{
...
double TickTransactionInferer::calculateConfidence(InferenceMethod method,
                                                   double bid_vol_change,
                                                   double ask_vol_change,
                                                   double price_change)
{
...
double TickTransactionInferer::calculateInferredVolume(
    double bid_vol, double prev_bid_vol, double ask_vol, double prev_ask_vol, double last_vol)
{
