...
inline const std::string& strategyTypeName(ArbitrageStrategy t)
{
    static const std::string names[] = {
        ...
        "statistical_arb",
        "market_making",
        "mean_reversion" // HYBRID fallback
    };
...
struct SpreadPairConfig
{
    ...
    SpreadPairConfig()
        : spread_type(SpreadType::WEIGHTED), leg1_multiplier(300.0), leg2_multiplier(300.0), leg1_ratio(1.0),
          ...
          expiry_threshold(7), primary_strategy(ArbitrageStrategy::MEAN_REVERSION), use_hybrid(false),
          enhance_quoting(true), pause_z_threshold(3.0), widen_z_threshold(1.5)
    {}
};
...
struct SpreadState
{
    ...
    SpreadState()
        : leg1_price(0), leg2_price(0), current_spread(0), current_price(0), mid_price(0), bid_price(0), ask_price(0),
          total_volume(0), average_trade_size(0), buy_volume(0), sell_volume(0), spread_mean(0), spread_std(0),
          zscore(0), correlation(0), beta(1.0), half_life(0), leg1_position(0), leg2_position(0), spread_position(0),
          unrealized_pnl(0), realized_pnl(0), entry_spread(0), last_update(0), position_open_time(0), is_active(false),
          is_converging(false)
    {}
    ...
    {
        if (!hasPosition() || now_us <= position_open_time)
            return 0;
...
struct SpreadSignal
{
    ...
    SpreadSignal()
        : type(SpreadSignalType::NONE), confidence(0), suggested_size(0), entry_zscore(0), target_zscore(0),
          leg1_price(0), leg2_price(0), leg1_qty(0), leg2_qty(0), source(ArbitrageStrategy::MEAN_REVERSION),
          timestamp(0)
    {}

    /// Check if signal is actionable
    inline bool isActionable() const { return type != SpreadSignalType::NONE && confidence > 0.1; }
};
...
struct SpreadRiskMetrics
{
    ...
    SpreadRiskMetrics()
        : var_99(0), max_loss(0), leg1_exposure(0), leg2_exposure(0), net_exposure(0), convergence_risk(0),
          correlation_risk(0), beta_instability(0), liquidity_risk(0), days_to_expiry_leg1(0), days_to_expiry_leg2(0)
    {}
};
...
struct QuotingAdjustment
{
    ...
    QuotingAdjustment()
        : bid_skew_adjustment(0), ask_skew_adjustment(0), spread_multiplier(1.0), suppress_bid(false),
          suppress_ask(false), confidence(0)
    {}
};
...
struct SpreadFeatureVector
{
    ...
    SpreadFeatureVector()
        : zscore(0), zscore_change(0), correlation(0), correlation_change(0), beta(1), beta_stability(1),
          volatility_ratio(1), volume_imbalance(0), cost_of_carry(0), momentum(0), mean_reversion_speed(0)
    {}
};
...
struct StrategyWeights
{
    ...
    StrategyWeights() : mean_reversion(0.4), trend_following(0.2), pairs_trading(0.2), statistical_arb(0.2) {}
...
struct PairArbState
{
    ...
    PairArbState()
        : intent(ArbIntent::NONE), intent_set_tick(0), in_flight_qty(0), in_flight_direction(0), in_flight_set_time(0),
          last_derived_position(0), close_in_flight_qty(0), close_in_flight_set_time(0)
    {}
};
...
{
    return t == SpreadSignalType::CLOSE_LONG_SPREAD || t == SpreadSignalType::CLOSE_SHORT_SPREAD ||
           t == SpreadSignalType::STOP_LOSS || t == SpreadSignalType::TIMEOUT_EXIT || t == SpreadSignalType::REBALANCE;
}
...
struct ArbCloseConfig
{
    ...
    bool is_allowed(SpreadSignalType t) const
    {
        if (!enabled)
            return false;
        switch (t) {
        case SpreadSignalType::CLOSE_LONG_SPREAD:
            return allow_signals.close_long;
        case SpreadSignalType::CLOSE_SHORT_SPREAD:
            return allow_signals.close_short;
        case SpreadSignalType::TIMEOUT_EXIT:
            return allow_signals.timeout_exit;
        case SpreadSignalType::STOP_LOSS:
            return allow_signals.stop_loss;
        default:
            return false; // REBALANCE 等保持抑制
        }
    }
