...
PortfolioRiskSummary SpreadRiskManager::calculatePortfolioRisk() const
{
    ...
    for (const auto& kv : _pair_states) {
        ...
        if (state.is_active) {
            ...
            summary.total_exposure +=
                std::abs(state.leg1_position * state.leg1_price) + std::abs(state.leg2_position * state.leg2_price);
            // 流动性: 当日成交量相对最低要求的比例, 截断到 [0,1]
            liquidity_sum +=
                std::min(1.0, state.total_volume / std::max(1.0, static_cast<double>(_config.min_daily_volume)));
        }
    ...
    for (const auto& alert : _active_alerts) {
        if (alert.level == RiskAlert::Level::CRITICAL || alert.level == RiskAlert::Level::EMERGENCY) {
...
double SpreadRiskManager::getAllowedPositionSize(const std::string& pair_id) const
{
    ...
    double current_pos = (state_it != _pair_states.end()) ? std::abs(state_it->second.spread_position) : 0;
