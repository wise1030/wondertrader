...
class StrikeSpreadScanner : public IScanModule {
public:
    struct Config {
        std::string ident = "StrikeSpreadScanner";
        bool enable = true;
        double min_edge = 0.5;
    ...
    StrikeSpreadScanner(const Config& cfg, OptionTraderContextPtr ctx) : m_config(cfg), m_ctx(ctx) {}
    virtual ~StrikeSpreadScanner() {}
