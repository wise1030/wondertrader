...
public:
    struct Config {
        std::string ident = "SyntheticFutureScanner";
        bool enable = true;
        double min_edge = 0.5;
    ...
    SyntheticFutureScanner(const Config& cfg, OptionTraderContextPtr ctx) : m_config(cfg), m_ctx(ctx) {}
    virtual ~SyntheticFutureScanner() {}
